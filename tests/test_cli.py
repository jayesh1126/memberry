...
from memberry import cli  # noqa: E402


def test_parser_dispatches_each_subcommand():
    parser = cli.build_parser()

    assert parser.parse_args(["ingest", "--repo", "x"]).func is cli._cmd_ingest
    assert parser.parse_args(["recall", "q"]).func is cli._cmd_recall
    assert parser.parse_args(["improve"]).func is cli._cmd_improve
    assert parser.parse_args(["forget"]).func is cli._cmd_forget
    assert parser.parse_args(["serve"]).func is cli._cmd_serve
...
def test_lifecycle_functions_are_coroutines():
    from memberry import lifecycle
...
def test_recall_handles_missing_dataset_gracefully(monkeypatch):
    ...
    from memberry import recall as recall_mod
    from memberry.config import load_settings
